...
def test_equivalence_ratio_addl_species(phi):
    ...
    reactants = equivalence_ratio(
        gas, phi, fuel, oxidizer, complete_prod, additional_species
    )
...
def test_eq_ratio_incomplete_prods():
    ...
    with pytest.warns(
        Warning, match="One or more products of incomplete combustion were"
    ):
...
def test_eq_ratio_raise_missing_elem_cprod():
    ...
    with pytest.raises(
        CanSenError, match=r"Must specify all elements in the fuel \+ oxidizer"
    ):
        equivalence_ratio(gas, phi, fuel, oxidizer, complete_prod, {})
