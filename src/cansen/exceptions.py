...
class MultipleProblemError(KeywordError):
    ...
    def __str__(self) -> str:
        return repr(
            "Error: more than one problem type keyword was specified."
...
class UnsupportedKeyword(Warning):
    ...
    def __str__(self) -> str:
        return repr(
            "Warning: keyword(s) {key1} is not supported yet and "
...
class MissingReqdKeywordError(KeywordError):
    ...
    def __str__(self) -> str:
        return repr("Error: Required keyword {} is missing.".format(self.keywords))
