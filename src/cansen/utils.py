...
parser.add_argument(
    "-V", "--version", action="store_true", help="Show the version of CanSen and quit"
)

parser.add_argument(
    "-i", "--input", type=str, help="The simulation input file in SENKIN format."
)

output_filename = parser.add_argument(
    "-o", "--output", type=str, default="output.out", help="The text output file."
)

save_filename = parser.add_argument(
    "-x", "--save", type=str, default="save.hdf", help="The binary save output file."
)

mech_filename = parser.add_argument(
    "-c",
    "--chem",
    type=str,
    default="chem.xml",
    help="The chemistry input file, in either CHEMKIN," " Cantera CTI or CTML format.",
)
...
def read_input_file(input_file: List[str]) -> dict:
    ...
    if "problemType" not in keywords:
        raise MissingReqdKeywordError(
            "CONP", "CONV", "VPRO", "CONT", "ICEN", "TPRO", "COTV", "TTIM", "VTIM"
        )
        ...
        if "stroke_length" in keywords:
            log.info(
                "Info: 'STROKE' was specified, and will be used for the "
                "stroke length regardless of other parameters."
            )
        elif all(key in keywords for key in ("swept_volume", "cyl_bore")):
            log.info(
                "Info: Using swept volume and cylinder bore to "
                "calculate stroke length."
            )
            keywords["stroke_length"] = (
                keywords["swept_volume"] * 4 / (pi * keywords["cyl_bore"] ** 2)
            )
        elif all(key in keywords for key in ("comp_ratio", "clear_volume", "cyl_bore")):
            log.info(
                "Info: Using compression ratio, clearance volume, and "
            ...
            raise MissingReqdKeywordError(
                "STROKE", "VOLD", "BORE", "CMPR", "VOLC", "CRAD"
            )

        # Handle the various ways to calculate the initial volume
        if "reactorVolume" in keywords:
            log.info(
                "Info: The initial reactor volume was specified by the VOL "
                "keyword and this value will be used regardless of other "
                "settings."
            )
        elif all(
            key in keywords for key in ("swept_volume", "clear_volume", "comp_ratio")
        ):
            raise KeywordError(
                "Only two of 'VOLD', 'VOLC', and 'CMPR' may be " "specified."
            )
        elif all(key in keywords for key in ("swept_volume", "clear_volume")):
            log.info(
                "Info: Computing initial reactor volume from the swept "
                "volume and the clearance volume."
            )
            keywords["reactorVolume"] = (
                keywords["swept_volume"] + keywords["clear_volume"]
            )
        elif all(key in keywords for key in ("comp_ratio", "clear_volume")):
            log.info(
                "Info: Computing initial reactor volume from the "
                "compression ratio and clearance volume."
            )
            keywords["reactorVolume"] = (
                keywords["comp_ratio"] * keywords["clear_volume"]
            )
            ...
            log.info(
                "Info: Computing initial reactor volume from the cylinder "
                "bore, stroke length, and clearance volume."
            )
            keywords["reactorVolume"] = (
                pi / 4 * keywords["cyl_bore"] ** 2 * keywords["stroke_length"]
                + keywords["clear_volume"]
            )
        else:
            raise MissingReqdKeywordError("VOLD", "VOLC", "CMPR", "BORE", "VOL")

        # Handle the ways to calculate the rod length to radius ratio
        if "rod_radius_ratio" in keywords:
            log.info(
                "Info: The connecting rod length to crank radius ratio was "
                "specified by the 'LOLR' keyword and this value will be "
                "used regardless of other settings."
            )
        elif all(key in keywords for key in ("connect_rod_len", "crank_radius")):
            log.info(
                "Info: Using given connecting rod length and crank radius "
                "to compute the ratio."
            )
            keywords["rod_radius_ratio"] = (
                keywords["connect_rod_len"] / keywords["crank_radius"]
            )
        else:
    ...
    if "reactorVolume" not in keywords:
        ...
        raise MissingKeyword("No reactor volume specified, assuming " "1.0 cm**3.")
    ...
    if reactants and (
        oxidizer
        or fuel
        or complete_products
        or additional_species
        or ("eqRatio" in keywords)
    ):
        ...
        raise KeywordError(
            "If EQUI is specified, all of FUEL, OXID and " "CPROD must be as well."
        )
    elif reactants:
...
def equivalence_ratio(
    ...
    complete_products: List[str],
    additional_species: Dict[str, float],
) -> Dict[str, float]:
    ...
    for el in gas.element_names:
        ...
        if (cprod_tot > 0 and fuel_tot == 0 and oxid_tot == 0) or (
            ...
            raise CanSenError(
                "Must specify all elements in the fuel + oxidizer "
